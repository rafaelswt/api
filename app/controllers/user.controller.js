...
exports.criarvaga = async (req, res) => {
  try {
    ...
    const vaga = new Vaga({
      ...
      user: req.userId,
      exclusivo_agencia: !!req.userRoles.includes('ROLE_AGENCY'),
    ...
    if (user) {
      ...
      paypal.payment.execute(paymentId, details, (error, payment) => {
        if (error) {
          ...
          let paymentStatusField // Declaração da variável paymentStatusField
          let caminho // Declaração da variável caminho
          ...
          switch (payment.transactions[0].item_list.items[0].name) {
            ...
            case 'Vaga Patrocinada':
              ...
              Vaga.findByIdAndUpdate(vagaId, { vaga_patrocinada: true }, { new: true }, (err) => {
                if (err) {
                  console.error(err)
                  res.status(500).send('Error processing payment')
                } else {
            ...
            { _id: userId },
            {
              ...
              $push: { // add an entry to the purchase history array
                purchaseHistory: {
                  ...
                  paymentId // add payment ID to the purchase history
                }
              }
            },
            { new: true },
